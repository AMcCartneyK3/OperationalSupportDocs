...
* Download the installation manifest from https://raw.githubusercontent.com/argoproj/argo-cd/stable/manifests/install.yaml and save within the argocd directory.
## 1. Install Argo CD
...
Also available in Mac Homebrew (untested):
...
!!! warning
    It is strongly advised to change the initially generated administrative password
...
> For completeness, Argo CD 1.8 and earlier
...
```bash
argocd login <ARGOCD_SERVER>  # e.g. localhost:8080 or argocd.node4.com
```
...
Rather than retrieving the auto-generated password, we can secure the platform and remove a known issue whereby the initial password is not valid with the following steps:

Generate a new Bcrypt encrypted hash using a service such as: https://bcrypt-generator.com/

Then path the secret using:

```bash
kubectl --kubeconfig=856-mgnt-config -n argocd patch secret argocd-secret \
  -p '{"stringData": {
    "admin.password": "[result of Bcrypt encrypted hashing]",
    "admin.passwordMtime": "'$(date +%FT%T%Z)'"
  }}'
```

Now, using the username `admin` and the password from above, login to Argo CD's IP or hostname:
...
!!! note
    ...
    `argocd app create k3-ewebshop --repo https://github.com/argoproj/argocd-example-apps.git --path guestbook --dest-server https://kubernetes.default.svc --dest-namespace default`
...
Give your app the name `k3-ewebshop`, use the project `default`, and leave the sync policy as `Manual`:
...
Once the k3-ewebshop application is created, you can now view its status:

```bash
$ argocd app get k3-ewebshop
Name:               k3-ewebshop
Server:             https://kubernetes.default.svc
Namespace:          default
URL:                https://10.97.164.88/applications/k3-ewebshop
...
apps   Deployment  default    k3-ewebshop  OutOfSync  Missing
       Service     default    k3-ewebshop  OutOfSync  Missing
```
